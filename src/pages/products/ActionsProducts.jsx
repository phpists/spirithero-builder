...
import "./ProductsStyles.css"; // якщо стилі окремо
...
function ActionsProducts({ activeTab }) {
    ...
    return (
        <div className="block-actions-products">
            <div className="block-sort-actions" onClick={toggleSortDropdown}>
                <p style={{ color: '#605767', fontSize: '14px', fontWeight: '500' }}>Sort by</p>
                <span className="sort-selected">
                    {selectedSort} <i className={`bi bi-chevron-${sortOpen ? 'up' : 'down'}`}></i>
                </span>
                ...
                {sortOpen && (
                    <div className="sort-dropdown">
                        {sortOptions.map((option, idx) => (
                            <div
                                key={idx}
                                className={`sort-item ${option === selectedSort ? 'active' : ''}`}
